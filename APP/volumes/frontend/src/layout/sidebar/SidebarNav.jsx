...
/* ── Iconos inline SVG (stroke-only, extraídos del Draft) ── */
const Icon = ({ d, d2 }) => (
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: "/reception", label: "Actas de Recepción",  icon: <Icon d="M19 12H9M13 6l-6 6 6 6M15 5h5v14h-5z" /> },
...
export function SidebarNav({ collapsed }) {
  ...
  return (
    <nav className="flex flex-1 flex-col gap-4" aria-label="Navegación principal">
      {NAV_GROUPS.map((group, gi) => (
        <SidebarMenuGroup key={gi} label={group.label} collapsed={collapsed}>
