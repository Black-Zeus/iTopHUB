...
function groupItemsByAssetClass(items = []) {
  return items.reduce((groups, item) => {
    const classLabel = String(item?.asset?.className || "").trim() || "Sin clasificacion";
    const existingGroup = groups.find((group) => group.classLabel === classLabel);
    if (existingGroup) {
...
export function createFormFromDetail(detail, bootstrap) {
  return {
    ...
    normalizationMode: detail.normalizationMode || "",
    normalizationParams: detail.normalizationParams || {},
...
export function HandoverEditorSections({
  ...
  showReceiverSummary = true,
  documentExtraContent = null,
  ...
  const topPanelsExpanded = !collapsedSections.document && (!showReceiverSection || !collapsedSections.receiver) && (!requiresSourceResponsible || !collapsedSections.source);
  ...
  const renderAssetCard = (item, { fullHeight = false } = {}) => {
    ...
    return (
      <Panel key={assetId} className={`min-w-0 bg-[var(--bg-app)] ${fullHeight ? "h-full" : ""}`.trim()}>
        <div className="grid min-w-0 gap-4">
          <div className="flex items-start justify-between gap-4">
            ...
            {!readOnly ? (
              <Button size="sm" variant="secondary" className="shrink-0 self-start" onClick={() => requestRemoveAssetFromForm(item.asset)}>
    ...
    <div className="grid gap-5">
      <div className={`grid gap-5 xl:grid-cols-2 ${topPanelsExpanded ? "xl:items-stretch" : "xl:items-start"}`}>
        ...
        <div className="grid gap-5">
          {topRightSection ? (
            <EditorSectionPanel
              eyebrow={topRightSection.eyebrow}
              title={topRightSection.title}
              helper={topRightSection.helper}
              isCollapsed={Boolean(collapsedSections[topRightSection.sectionKey])}
              onToggle={() => toggleSection(topRightSection.sectionKey)}
            >
          ...
          >
            <div className="grid gap-4">
              ...
              {receiverExtraContent}

              {showReceiverSummary && form.receiver ? (
                <div className="grid gap-3">
              ...
              {showReceiverSummary && allowAdditionalReceivers && form.additionalReceivers?.length ? (
                <div className="grid gap-3">
...
{!readOnly ? (
                ...
                <div className="grid gap-3">
                  {assetResults.map((asset) => {
                    const restrictionMessage = getAssetAssignmentRestriction(asset, {
                      assetSelectionMode: assetRestrictionMode,
      ...
      {useGroupedAssetLayout ? (
        groupedItems.length ? (
          <div className="grid gap-4">
            {groupedItems.map((group) => (
              <section key={group.classLabel} className="grid gap-4 rounded-[24px] border border-[var(--border-color)] bg-[var(--bg-panel)] p-4 md:p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-[0.72rem] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">Grupo de activos</p>
                    <h3 className="mt-1 text-base font-semibold text-[var(--text-primary)]">{group.classLabel}</h3>
                  </div>
                ...
                <div className="grid items-start gap-4 lg:grid-cols-2 min-[1680px]:grid-cols-3 min-[2200px]:grid-cols-4">
                  {group.items.map((item) => renderAssetCard(item, { fullHeight: true }))}
                </div>
              </section>
            ))}
          </div>
        ...
        form.items?.length ? (
          <div className="grid gap-4">
            {form.items.map((item) => renderAssetCard(item))}
          </div>
        ) : null
      )}
    </div>
