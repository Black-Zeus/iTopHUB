...
- The institutional legend `Documento emitido desde iTop HUB` must no longer be hardcoded in templates; `Configuracion > Documentos` owns its enable/disable toggle and editable text for both the main acta subtitle and the issuer signature fallback.
