...
- The row-actions area in handover lists must render in rows of up to 3 buttons. When a second row has fewer actions, those buttons must expand to the available width instead of preserving empty slots.
...
- The QR modal in the authenticated Hub must only show the QR itself plus session state. It must not expose raw links or desktop shortcuts, because the intended journey is mobile-first.
- Public signature branding must come from `Configuracion > Organizacion` and QR session behavior from `Configuracion > Firma QR`, so the frontend should consume those values from backend payloads instead of deriving URLs from `window.location`.
