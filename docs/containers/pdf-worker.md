...
- Handover jobs should consume public helpers from `modules/handover/payloads.py` and `modules/handover/document_templates.py`; avoid importing private helpers from `modules/handover/service.py` so rendering and orchestration can evolve independently.
- In `dev`, handover emit processing should reload the local renderer and backend handover helpers on each job invocation so bind-mounted refactors do not leave `pdf-worker` stuck on stale in-memory code until the next manual restart.
- That dev reload path must include shared settings or handover-type helper modules when emission logic starts depending on new backend configuration helpers; otherwise the worker may keep stale imports and fail queued jobs after a refactor.
